...
module Ohua.Core.Serialize.JSON
    ( encode
    , eitherDecode
    ) where

import Ohua.Core.Prelude hiding (Options)
...
import Ohua.Core.DFLang.Lang (NodeType(..))
...
instance ToJSON PrivateFunRefLitWrapper where
    toJSON l@(Ohua.Core.Serialize.JSON.FunRefLit _) = genericToJSON baseOptions l
    toJSON (OtherLit o) = toJSON o

instance FromJSON PrivateFunRefLitWrapper where
    parseJSON v =
        (do l@Ohua.Core.Serialize.JSON.FunRefLit {} <- genericParseJSON baseOptions v
            pure l) <|>
...
instance ToJSON OutGraph where
    toJSON =
        ...
        (mapOutGraphLit %~ (\case
             Ohua.Core.Prelude.FunRefLit (FunRef l _) ->
                 Ohua.Core.Serialize.JSON.FunRefLit l
...
instance FromJSON OutGraph where
    parseJSON =
        fmap
            (mapOutGraphLit %~ \case
                 Ohua.Core.Serialize.JSON.FunRefLit l ->
                     Ohua.Core.Prelude.FunRefLit (FunRef l Nothing)
                 OtherLit l -> l) .
