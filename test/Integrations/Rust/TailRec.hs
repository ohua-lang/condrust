                ...
                expected <- showCode "Expected:"
                    [sourceFile|
                        ...
                        fn test() -> i32 {
                          let (c_0_tx, c_0_rx) = std::sync::mpsc::channel();
                          let (a_0_tx, a_0_rx) = std::sync::mpsc::channel();
                          let (one_0_1_tx, one_0_1_rx) = std::sync::mpsc::channel();
                          let (two_0_1_tx, two_0_1_rx) = std::sync::mpsc::channel();
                          let (ctrl_0_0_tx, ctrl_0_0_rx) = std::sync::mpsc::channel();
                          let (j_0_0_tx, j_0_0_rx) = std::sync::mpsc::channel();
                          ...
                          tasks
                            .push(Box::new(move || -> _ {
                              loop {
                                let var_0 = i_0_0_rx.recv()?;
                                let var_1 = j_0_0_rx.recv()?;
                                let k_0_0 = h2(var_0, var_1);
                                k_0_0_tx.send(k_0_0)?
                              }
                            }));
                          tasks
                          ...
                          tasks
                            .push(Box::new(move || -> _ {
                              ...
                              let init_0 = 2;
                              let init_1 = 4;
                              one_0_1_tx.send(init_0)?;
                              two_0_1_tx.send(init_1)?;
                              while a_0_rx.recv()? {
                                k_0_0_rx.recv()?;
                                let ctrlSig = (true, 1);
                                ctrl_0_0_tx.send(ctrlSig)?;
                                let loop_res_0 = i_0_0_rx.recv()?;
                                let loop_res_1 = j_0_0_rx.recv()?;
                                one_0_1_tx.send(loop_res_0)?;
                                two_0_1_tx.send(loop_res_1)?;
                                ()
                              };
                              let ctrlSig = (false, 0);
                              ctrl_0_0_tx.send(ctrlSig)?;
                              let finaResult = k_0_0_rx.recv()?;
                              c_0_tx.send(finalResult)?
                            }));
                          tasks
                            .push(Box::new(move || -> _ {
