...
fn test(i: i32) -> () {
  ...
  let (x_lit_1_0_0_tx, x_lit_1_0_0_rx) = std::sync::mpsc::channel::<  i32,>();
  let (a_0_0_tx, a_0_0_rx) = std::sync::mpsc::channel::<  (),>();
  let (size_0_tx, size_0_rx) = std::sync::mpsc::channel::<  usize,>();
  let (x_lit_0_0_0_tx, x_lit_0_0_0_rx) =
    std::sync::mpsc::channel::<  Vec<  (),>,>();
  let mut tasks: Vec<  Box<  dyn FnOnce() -> Result<(), RunError> + Send,>,> =
    Vec::new();
  tasks
    .push(Box::new(move || -> _ {
      loop {
        ...
        for _ in 0 .. num {
          let data = a_0_0_rx.recv()?;
          collection.push(data)
        };
        x_lit_0_0_0_tx.send(collection)?
      }
    }));
  tasks
    .push(Box::new(move || -> _ {
      loop { x_lit_0_0_0_rx.recv()?; result_0_0_0_tx.send(())? }
    }));
  tasks
    .push(Box::new(move || -> _ {
      loop { x_lit_1_0_0_rx.recv()?; a_0_0_tx.send(())? }
    }));
  tasks
    .push(Box::new(move || -> _ {
      loop {
        let mut var_0 = d_0_rx.recv()?;
        let x_lit_1_0_0 = var_0.gs(5);
        x_lit_1_0_0_tx.send(x_lit_1_0_0)?;
        ()
      }
    }));
  tasks
    .push(Box::new(move || -> _ {
      let mut stream_0_0_0 = crate::funs::iter_states();
      let hasSize =
        {
      ...
      Ok(if hasSize {
        ...
        let mut size = 0;
        for d in stream_0_0_0 { d_0_tx.send(d)?; size = size + 1; () };
        size_0_tx.send(size)?;
        ()
      })
    }));
...
fn test(i: i32) -> () {
      ...
      loop {
        ...
        while !renew {
          ...
          for _ in 0 .. count {
            ...
            let x_lit_1_0_0 = io_0_0_1_0.gs(var_1);
            x_lit_1_0_0_tx.send(x_lit_1_0_0)?;
            ()
          };
  ...
  tasks
    .push(Box::new(move || -> _ {
      loop { x_lit_1_0_0_rx.recv()?; b_0_0_tx.send(())? }
    }));
  tasks
    .push(Box::new(move || -> _ {
  ...
  tasks
    .push(Box::new(move || -> _ {
      let io_0_0_1 = crate::funs::State::new_state(i);
