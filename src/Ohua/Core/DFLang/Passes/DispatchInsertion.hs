...
fromApp :: DFApp fTy embExpr annot ty -> UsageSite
...
insertDispatch :: forall m embExpr annot ty. MonadGenBnd m => MonadOhua m => NormalizedDFExpr embExpr annot ty -> m (NormalizedDFExpr embExpr annot ty)
insertDispatch e = evalStateT (transformExprM go e) HM.empty
  where
    go :: NormalizedDFExpr embExpr annot ty -> StateT (FreeVars ty) m (NormalizedDFExpr embExpr annot ty)
    ...
    maybeDispatch :: NormalizedDFExpr embExpr annot ty -> Maybe (OutData bndType ty) -> StateT (FreeVars ty) m (NormalizedDFExpr embExpr annot ty, Maybe (OutData bndType ty))
    maybeDispatch cont dOut =
      let f :: OutData bndType ty -> StateT (FreeVars ty) m (NormalizedDFExpr embExpr annot ty, Maybe (OutData bndType ty))
          f out = do
            (c', out') <- dispatchOutput cont out
            return (c', Just out')
       in maybeM (return (cont, dOut)) f $ return dOut

    dispatchOutput :: NormalizedDFExpr embExpr annot ty -> OutData bndType ty -> StateT (FreeVars ty) m (NormalizedDFExpr embExpr annot ty, OutData bndType ty)
    ...
    checkAndDispatch :: NormalizedDFExpr embExpr annot ty -> ATypedBinding bndTy ty -> StateT (FreeVars ty) m (NormalizedDFExpr embExpr annot ty, NonEmpty (ATypedBinding bndTy ty))
    ...
    insertDispatch' :: NormalizedDFExpr embExpr annot ty -> ATypedBinding bndTy ty -> SNat ('Succ n) -> m (NormalizedDFExpr embExpr annot ty, NonEmpty (ATypedBinding bndTy ty))
