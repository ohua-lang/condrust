...
runCorePasses :: (MonadOhua m) => NormalizedExpr embExpr annot ty -> m (NormalizedDFExpr embExpr annot ty)
runCorePasses = removeNth

finalPasses :: (MonadOhua m) =>  NormalizedDFExpr embExpr annot ty -> m (NormalizedDFExpr embExpr annot ty)
...
removeNth :: forall embExpr annot ty m. MonadOhua m => NormalizedExpr embExpr annot ty-> m (NormalizedDFExpr embExpr annot ty)
removeNth expr = do
  checkSSA expr
  ...
  where
    ...
    f = \case
      ...
      where
        go ::
          App a embExpr annot ty ->
          NormalizedExpr embExpr annot ty ->
          State
    ...
    toDFAppFun :: App a embExpr annot ty
               -> State
               (HM.HashMap (TypedBinding ty) (NonEmpty (Integer, TypedBinding ty)))
...
checkDefinedUsage :: MonadOhua m => NormalizedDFExpr embExpr annot ty -> m ()
checkDefinedUsage expr = evalStateT (f expr) HS.empty
  where
    f (DFLang.Let app cont) = checkAndDescend app cont
    f _ = return ()
    checkAndDescend :: (MonadOhua m, MonadState (HS.HashSet (TypedBinding ty)) m) => DFApp a embExpr annot ty -> NormalizedDFExpr embExpr annot ty -> m ()
...
checkSSA :: forall b embExpr annot ty m. MonadOhua m => DFLang.Expr b embExpr annot ty -> m ()
checkSSA e =
  mapM_
      ...
      HM.toList $
        HM.filter ((> 1) . length) $
          foldl
            ...
            HM.empty
            (allOuts e)
    allOuts :: DFLang.Expr b embExpr annot ty -> [(TypedBinding ty, Text)]
...
-- lowerALang :: MonadOhua m => Expression -> m (NormalizedExpr embExpr annot ty)
-- lowerALang expr
...
lowerToDF :: MonadOhua m => ALang.Expr embExpr annot ty -> m (NormalizedExpr embExpr annot ty)
lowerToDF expr = evalStateT (transfer' expr) HS.empty
  where
    transfer' ::
      ...
      m (NormalizedExpr embExpr annot ty)
...
handleDefinitionalExpr' :: forall embExpr annot ty m.
  (MonadState (HS.HashSet (TypedBinding ty) ) m, MonadOhua m) =>
  TypedBinding ty ->
  ALang.Expr embExpr annot ty ->
  ALang.Expr embExpr annot ty ->
  ...
  where
    go args e =
      case e of
        ...
        x -> failWith $ "Expected Apply or Var but got: " <> show (x :: ALang.Expr embExpr annot ty)
...
expectVar :: (HasCallStack, MonadError Error m) => OhuaType ty Resolved -> ALang.Expr embExpr annot ty -> m (DFVar 'Data embExpr annot ty)
...
expectStateBnd :: (HasCallStack, MonadError Error m) => ALang.Expr embExpr annot ty -> m (ATypedBinding 'State ty)
