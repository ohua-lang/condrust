...
data DFVar (semType :: BindingType) (embExpr::Type) (ty :: Type) :: Type where
  DFEnvVar :: OhuaType ty Resolved-> Lit embExpr ty Resolved -> DFVar 'Data embExpr ty
  DFVar :: ATypedBinding a ty -> DFVar a embExpr ty
...
data App (f :: FunANF) (embExpr :: Type)(ty :: Type) :: Type where
  ...
  PureFun :: DFVar bTy embExpr ty -> FunRef ty  Resolved -> NonEmpty (DFVar 'Data embExpr ty) -> App 'Fun embExpr ty
  StateFun ::
    (Maybe (ATypedBinding 'State ty), ATypedBinding 'Data ty) ->
    FunRef ty  Resolved->
    DFVar 'State embExpr ty ->
    NonEmpty (DFVar 'Data embExpr ty) ->
    App 'ST embExpr ty

-- | The applicative normal form with the ops resolved.
--   (a function with output destructuring and dispatched result)
data DFApp (f :: FunANF) (embExpr :: Type) (ty :: Type) :: Type where
  PureDFFun :: OutData b ty -> FunRef ty  Resolved-> NonEmpty (DFVar 'Data embExpr ty) -> DFApp 'Fun embExpr ty
  StateDFFun ::
  ...
  -- (final) result out
    ...
    Vec n (OutData b ty) ->
    -- | initial inputs
    Vec n (DFVar a embExpr ty) ->
    -- | recursion args inputs
    Vec n (DFVar a embExpr ty) ->
    -- | recursion condition input
    DFVar 'Data embExpr ty ->
    -- | (final) result in
    DFVar 'Data embExpr ty ->
    DFApp 'Fun embExpr ty -- DFApp 'BuiltIn ty , commented BuiltIn out cause it's currently useless and messes up my monads
  ...
  SMapFun ::
    ...
    -- | data in
    DFVar a embExpr ty ->
    DFApp 'Fun embExpr ty -- FIXME would be this: DFApp 'BuiltIn ty (fix when all IRs are fixed and I do not need to convert a Fun into a BuiltIn)
...
data Expr (fun :: FunANF -> Type -> Type -> Type) (embExpr :: Type) (ty :: Type) :: Type where
  -- Question: I don't understand the order of that Let term? What are the arguments supposed to represent?
  Let :: (Show (fun fa embExpr ty), Function fun) => fun fa embExpr ty -> Expr fun embExpr ty -> Expr fun embExpr ty
  -- FIXME this also should probably have a BindingType!
  -- Well it should first of all have a Type type :-/
  Var :: ATypedBinding b ty -> Expr fun embExpr ty
...
outsApp :: App fty embExpr ty -> NonEmpty (TypedBinding ty)
...
outsDFApp :: DFApp fty embExpr ty -> [TypedBinding ty]
...
insApp :: App fty embExpr ty -> [TypedBinding ty]
...
insDFApp :: DFApp fty embExpr ty -> [TypedBinding ty]
...
extractBndsFromInputs :: [DFVar bTy embExpr ty] -> [TypedBinding ty]
extractBndsFromInputs =
  mapMaybe (\case DFVar atbnd -> Just $ unwrapTB atbnd; _ -> Nothing)


insAndTypesDFApp :: DFApp fty embExpr ty -> [TypedBinding ty]
...
extractBndsAndTypesFromInputs :: [DFVar bTy embExpr ty] -> [TypedBinding ty]
extractBndsAndTypesFromInputs =
  mapMaybe (\case (DFVar atbnd) -> Just $ unwrapTB atbnd; _ -> Nothing)

fnApp :: App fty embExpr ty -> QualifiedBinding
...
unwrapVarType :: DFVar bTy embExpr ty -> OhuaType ty Resolved
...
unwrapVarBnd :: DFVar bTy embExpr ty -> Binding
...
unwrapVarTB :: DFVar bTy embExpr ty -> TypedBinding ty
...
deriving instance Show (DFVar semTy embExpr ty)

deriving instance Eq (DFVar semTy embExpr ty)

deriving instance Show (App fn embExpr ty)

-- deriving instance Eq (App fn embExpr ty)
...
-- instance Eq (App fn embExpr ty) where
...
type NormalizedExpr embExpr ty = Expr App embExpr ty

type NormalizedDFExpr embExpr ty = Expr DFApp embExpr ty

deriving instance Show (NormalizedExpr embExpr ty)
...
deriving instance Show (DFApp embExpr ty a)
...
-- instance Eq (DFApp fTy embExpr ty) where
...
-- instance Plated (NormalizedDFExpr embExpr ty) where plate = gplate
...
transformExpr :: forall embExpr ty. (NormalizedDFExpr embExpr ty -> NormalizedDFExpr embExpr ty) -> NormalizedDFExpr embExpr ty -> NormalizedDFExpr embExpr ty
transformExpr f = runIdentity . transformExprM go
  where
    go :: NormalizedDFExpr embExpr ty -> Identity (NormalizedDFExpr embExpr ty)
    go = pure . f

-- | This is a bottom-up traversal
transformExprM :: Monad m => (NormalizedDFExpr embExpr ty -> m (NormalizedDFExpr embExpr ty)) -> NormalizedDFExpr embExpr ty -> m (NormalizedDFExpr embExpr ty)
transformExprM f (Let app cont) = f . Let app =<< transformExprM f cont
transformExprM f v@(Var _) = f v

-- | This is a top-down traversal
transformExprTDM :: Monad m => (NormalizedDFExpr embExpr ty -> m (NormalizedDFExpr embExpr ty)) -> NormalizedDFExpr embExpr ty -> m (NormalizedDFExpr embExpr ty)
...
mapFunsM :: Monad m => (forall fTy. DFApp fTy embExpr ty -> m (DFApp fTy embExpr ty)) -> NormalizedDFExpr embExpr ty -> m (NormalizedDFExpr embExpr ty)
...
mapFuns :: (forall fTy. DFApp fTy embExpr ty -> DFApp fTy embExpr ty) -> NormalizedDFExpr embExpr ty -> NormalizedDFExpr embExpr ty
mapFuns f e = runIdentity (mapFunsM (pure . f) e)

universe' :: NormalizedDFExpr embExpr ty -> [NormalizedDFExpr embExpr ty]
...
length :: Expr semTy embExpr ty -> V.Nat
length (Let _ cont) = Succ $ length cont
length Var {} = Zero

countBindings :: NormalizedDFExpr embExpr ty -> V.Nat
...
usedBindings :: NormalizedDFExpr embExpr ty -> [TypedBinding ty]
...
substitute :: forall embExpr ty. SubstitutionStrategy -> (TypedBinding ty, TypedBinding ty) -> NormalizedDFExpr embExpr ty -> NormalizedDFExpr embExpr ty
substitute strat (from,to) e = evalState (mapFunsM go e) False
  where
    go :: DFApp fTy embExpr ty -> State Bool (DFApp fTy embExpr ty)
    ...
    replace :: DFVar bTy embExpr ty -> State Bool (DFVar bTy embExpr ty)
    ...
    check :: DFApp fTy embExpr ty -> State Bool (DFApp fTy embExpr ty)
    check app = checkDefined app >> return app

    checkDefined :: DFApp fTy embExpr ty -> State Bool ()
    checkDefined f =
