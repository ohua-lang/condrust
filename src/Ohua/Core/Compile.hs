...
pipeline :: (Show embExpr, Show annot) => CustomPasses embExpr annot ty -> ALang.Expr embExpr annot ty -> OhuaM (NormalizedDFExpr embExpr annot ty)
...
compile :: (ErrAndLogM m, Show embExpr, Show annot) => Options -> CustomPasses embExpr annot ty -> ALang.Expr embExpr annot ty-> m (NormalizedDFExpr embExpr annot ty)
...
checkHigherOrderFunctionSupport :: MonadOhua m => ALang.Expr embExpr annot ty -> m ()
