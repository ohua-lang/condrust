...
recurSf :: OhuaType ty Resolved -> Expr embExpr annot ty
...
ySf :: OhuaType ty Resolved -> Expr embExpr annot ty
ySf ty = pureFunction y $ FunType (Right $ ty :| []) ty

-- see above 
recurFunPureFunction :: OhuaType ty Resolved -> Expr embExpr annot ty
...
findRecCall ::
    ...
    -> ReaderT Bool m (HS.HashSet (TypedBinding ty), Expr embExpr annot ty)
...
rewriteAll :: (MonadGenBnd m, Show embExpr) => Expr embExpr annot ty -> m (Expr embExpr annot ty)
...
rewriteCallExpr :: forall m embExpr annot ty.
       (MonadGenBnd m, Show embExpr) => Expr embExpr annot ty -> m (Expr embExpr annot ty)
rewriteCallExpr e = do
  ...
  where
    rewriteLastCond :: Expr embExpr annot ty -> Expr embExpr annot ty
