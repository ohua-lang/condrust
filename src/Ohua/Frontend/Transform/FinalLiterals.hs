...
noFinalLiterals e@(LetE _p _f l@LitE{}) = 
...
-- noFinalLiterals (StateFunE state stBnd mCall ) = noFinalLiterals state >> noFinalLiterals mCall
noFinalLiterals e@(StmtE _f l@LitE{}) =  throwError $ "The literal [" <> show l <> "] does not depend on a previously computed value in the expression \n" 
  <> show e
  <>"\nIn the host program this meant that it depends on the previous statements. This compiler does not yet support creating this dependency implicity. Hence, we kindly ask you to add it explicitly to your code."
