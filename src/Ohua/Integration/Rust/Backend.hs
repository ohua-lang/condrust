...
convertIntoBlock :: (Architecture arch, Lang arch ~ Language 'Rust) => arch -> TaskExpr RustTypeAnno -> Sub.Block
...
convertFunCall :: (Architecture arch, Lang arch ~ Language 'Rust, ty ~ B.Type (Lang arch))
               => arch -> QualifiedBinding -> [TCLang.TaskExpr RustTypeAnno] -> Sub.Expr
convertFunCall arch f args =
  case (binOp f, args) of
    ...
    _ -> case (unOp f, args) of
           ...
           _ ->
             Sub.Call f $ map (convertExpr arch) args
  where
    binOp = \case
